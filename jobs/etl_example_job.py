...
def extract_data(spark):
    """Load scripts from Parquet file format.
...
def load_data(df):
    """Collect scripts locally and write to CSV.
...
def create_test_data(spark, config):
    ...
    This function creates both both pre- and post- transformation scripts
    saved as Parquet files in tests/test_data. This will be used for
    unit tests as well as to load as part of the example ETL job.
    :return: None
    """
    # create example scripts from scratch
    local_records = [
    ...
    # create transformed version of scripts
    df_tf = transform_data(df, config['steps_per_floor'])

    # write transformed version of scripts to Parquet
    (df_tf
     .coalesce(1)
     .write
