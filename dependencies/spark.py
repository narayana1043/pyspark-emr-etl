...
def start_spark(app_name, master='local[*]', jar_packages=[],
                files=[], spark_config={}):
    ...
    # spark_sess.sparkContext().hadoopConfiguration().setSystemProperty("com.amazonaws.services.s3.enableV4", "true")
    #
    ...
    # spark_sess.sparkContext().hadoopConfiguration().hadoop_conf.set("fs.s3a.impl", "org.apache.hadoop.fs.s3a.S3AFileSystem")
    # spark_sess.sparkContext().hadoopConfiguration().hadoop_conf.set("fs.s3a.access.key", access_id)
    # spark_sess.sparkContext().hadoopConfiguration().hadoop_conf.set("fs.s3a.secret.key", access_key)

    spark_logger = spark_logging.Log4j(spark_sess)
    spark_logger.warn(spark_sess.sparkContext._conf.getAll())
